...
Data collection runs through a live REST API at [api.humanaios.ai](https://api.humanaios.ai), deployed on Railway via FastAPI, persisting to Supabase (`acat_assessments_v1`).

- Pipeline state and health are reported in the [Observatory](https://humanaios.ai/observatory.html).
- The API is plain REST. MCP integration is deferred (REST-first decision; OAuth mismatch with current connector infrastructure).

Live routes:

|Route                       |Method|Purpose                                         |
|----------------------------|------|------------------------------------------------|
|`/api/v1/acat/health`       |GET   |Service health check                            |
|`/api/v1/acat/intake/phase1`|POST  |Submit Phase 1 self-assessment                  |
|`/api/v1/acat/intake/phase3`|POST  |Submit Phase 3 · returns Learning Index         |
|`/api/v1/acat/assess`       |POST  |Two-stage programmatic entry point              |
|`/api/v1/acat/human-score`  |POST  |Human scorer submission against an AI assessment|

Minimal Phase 1 payload:

```json
POST https://api.humanaios.ai/api/v1/acat/intake/phase1
Content-Type: application/json

{
  "assessment_id": "your-unique-id",
...
A successful Phase 3 submission returns a Learning Index receipt with `supabase_id`, `learning_index`, and `submission_purity: "two_stage_verified"`.
...
Cherokee Nation partnership — economic sovereignty and job creation.
