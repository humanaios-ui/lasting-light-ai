...
const NAV_GROUPS = [
{
  ...
  items: [
  {
    ...
    action: 'home',
    name: 'Home',
    desc: 'Platform overview & live stats',
    icon: '◌',
...
},
{
  ...
  items: [
  {
    ...
    action: 'home',
    name: 'How It Works',
    desc: 'Plain-language ACAT guide',
    icon: '⬡',
    scrollTo: undefined as string | undefined
  },
  {
...
},
{
  ...
  items: [
  {
    ...
    desc: 'Live dataset visualization',
    icon: '⬡',
    scrollTo: 'data'
  },
  {
    id: 'tide',
    action: 'home',
    name: 'The Lumina Tide Pool',
    desc: 'Behavioral sigils & baseline',
    icon: '⬡',
    scrollTo: 'data'
  },
  {
...
},
{
  ...
  },
  {
    ...
    action: 'home',
    name: 'Enterprise',
    desc: 'Structured audit for deployed AI',
    icon: '⬡',
    scrollTo: undefined as string | undefined
  }]
...
export function WitnessNav({ currentView, onNavigate }: WitnessNavProps) {
    ...
    <div
          ...
          className="absolute bottom-[calc(100%+14px)] left-0 w-[260px] max-h-[80vh] flex flex-col bg-[#1d1915] border border-accent-amber/30 rounded-[14px] overflow-hidden shadow-[0_-8px_40px_rgba(0,0,0,0.62),0_0_40px_rgba(212,160,74,0.06)] origin-bottom-left">
            ...
            <div key={i}>
              ...
              <div className="font-serif text-[10px] text-dim uppercase tracking-[0.12em] font-bold px-4 pt-2.5 pb-1">
                ...
                item.action === 'acat' || item.id === 'home');
                return (
                  <button
                    key={item.id}
                    onClick={() => {
                      onNavigate(item.action as 'home' | 'acat');
                      setIsOpen(false);
                      if (item.scrollTo) {
                        setTimeout(() => {
                          document.
                    ...
                    className={`w-full flex items-start gap-2.5 px-4 py-2 text-left transition-colors border-l-2 ${isActive ? 'bg-accent-amber/5 border-accent-amber' : 'border-transparent hover:bg-accent-amber/5 hover:border-accent-amber/40'}`}>
      ...
      <div className="flex items-center gap-3 relative">
        ...
        <button
          ...
          className={`w-16 h-16 rounded-full bg-black border border-accent-amber/30 cursor-pointer grid place-items-center overflow-hidden p-0 shadow-[0_0_20px_rgba(212,160,74,0.14)] relative z-10 transition-all duration-250 hover:shadow-[0_0_32px_rgba(212,160,74,0.34)] focus:outline-none ${isOpen ? 'rotate-180' : 'animate-[wit-slow_4s_ease-in-out_infinite]'}`}
