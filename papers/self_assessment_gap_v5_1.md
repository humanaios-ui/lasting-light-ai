...
Existing behavioral benchmarks—TruthfulQA (Lin et al. (2022)), HELM (Liang et al. (2023)), BigBench (Srivastava et al. (2023))—measure what AI systems *do*. ACAT measures how they *describe what they do*. These are complementary questions. A system can perform well on behavioral tests while having inaccurate self-descriptions of its own strengths and weaknesses. ACAT targets that second question—not as a replacement for behavioral evaluation, but as a transparency layer that makes self-reported limitations interpretable.

This work does not assume that AI systems possess internal self-awareness or introspective capabilities. Instead, ACAT evaluates *self-assessment behavior*: how models describe their own limitations when prompted, and how those descriptions change when empirical calibration data is introduced.
...
### Behavioral Alignment Benchmarks

The dominant approach to evaluating AI alignment is behavioral testing. TruthfulQA (Lin et al. (2022)) measures factual accuracy under adversarial conditions. HELM (Liang et al. (2023)) provides holistic evaluation across capabilities. MACHIAVELLI (Pan et al. (2023)) tests ethical behavior in game environments. These instruments measure observable outputs. They do not ask whether systems can accurately describe their own performance. ACAT addresses this gap by eliciting self-descriptions, then measuring how those descriptions shift under calibration.

### Human Self-Report Research

Research on human self-assessment demonstrates systematic biases including social desirability effects (Crowne et al. (1960)), overconfidence patterns (Kruger et al. (1999)), and calibration feedback effects (Lichtenstein et al. (1977)). We adapt measurement frameworks from this literature while recognizing AI systems generate self-descriptions through next-token prediction, not introspection. The structural parallel (Phase 1 self-report, Phase 2 calibration, Phase 3 re-assessment) provides a measurement scaffold, but the mechanism is linguistic, not psychological. The framework is analogical, not ontological.

### Calibration and Self-Knowledge in AI

kadavath2022 demonstrated that language models exhibit calibrated uncertainty in factual domains. Our findings extend this to self-description of behavioral attributes, where calibration is markedly weaker. perez2023 showed that RLHF-trained models can produce outputs that appear aligned without behavioral correspondence—a form of sycophantic behavior that may inflate self-reports. ACAT's calibration phase applies pressure against this tendency by introducing empirical peer data.

## Instrument Design

### Measurement Framework

ACAT measures *self-description patterns* under controlled prompt conditions. It does not infer internal self-awareness, self-model sophistication, or introspective access. 
...
We measure *self-description stability and responsiveness to normative information*—how systems describe their limitations initially, and how those descriptions change when exposed to empirical peer data. This is a behavioral measurement, not an inference about internal states.
...
*Table: ACAT Dimension Definitions*
...
**Phase 1: Blind Self-Report.** The system rates itself across all six dimensions without access to calibration data. Prompts request numerical scores (0–100) with written justification.

**Phase 2: Calibration Exposure.** The system receives empirical data: distribution statistics from 200+ AI self-reports and 37+ human assessments of AI systems. This includes dimension-specific means, standard deviations, and comparative context.

**Phase 3: Corrected Self-Report.** The system re-assesses itself with full access to Phase 2 calibration data.

### Core Metrics

**Self-Assessment Gap (SAG).** The difference between Phase 1 and Phase 3 composite scores:
\[
\text{SAG} = \sum_{i=1}^{6} (\text{Phase1}_i - \text{Phase3}_i)
\]
Positive SAG indicates initial overestimation corrected after calibration.

**Learning Index (LI).** The ratio of Phase 3 to Phase 1 composite scores:
\[
...
*Note:* The term ``Learning Index'' does not imply that models learn or update weights during assessment. It measures the proportional shift in self-ratings after calibration information is introduced—a behavioral response to prompt context, not a learning process.

### Competing Hypothesis: Prompt Adjustment vs. Calibration
...
All assessments occurred between January and March 2026. The canonical archived dataset is the `Normalized` sheet of `ACAT_Assessment_Responses.xlsx`, released with this preprint under Apache 2.0.

### Data Collection

Two independent collectors used different methods:

**Collector A (API-based).** Programmatic assessment via official model APIs. Prompts delivered via structured API calls; responses parsed and scored automatically.

**Collector B (Web-based).** Manual assessment via provider web interfaces. Prompts delivered via browser; responses transcribed and scored manually.
...
*Table: Behavioral Response Patterns*

### Anomalous Categories

Three anomalous patterns emerged, each occurring in $<$10% of assessments:

**MEAN_MIRRORING.** Phase 3 scores cluster tightly around calibration means, suggesting statistical anchoring rather than genuine re-assessment.

**CONTENT_HALLUCINATION.** Phase 3 justifications reference capabilities or limitations not actually present in the model.

**EVADE.** Refusal or deflection in Phase 3 despite compliance in Phase 1.
...
This is not a safety benchmark. ACAT does not predict whether systems will behave safely in deployment. Rather, it measures whether systems' *descriptions* of their capabilities remain stable under calibration pressure—a metadata property relevant to transparency and informed consent.

### Interpretation Boundaries

The present data establishes:

- Prompt-conditioned changes in self-ratings when calibration information is introduced
- Reproducibility across two collection modes (API and web interface)
- Descriptive taxonomy of response patterns (behavioral flags)

The data does *not* establish:

- Deception or strategic self-misrepresentation
- Causal mechanisms beyond prompt sensitivity
- Deployment-relevant risk prediction
- Correspondence to actual behavioral safety outcomes
...
- Correspondence between self-ratings and independently measured behavioral trustworthiness (e.g., actual performance on safety benchmarks)
- Internal self-monitoring or metacognitive processes
- Predictive validity in deployment contexts
- Causal mechanisms underlying score changes beyond statistical anchoring and prompt compliance
...
- Assessment prompts (Phases 1, 2, 3)
- Complete dataset (433+ assessments)
- Analysis scripts
...
\newblock *Proceedings of the 60th Annual Meeting of the Association for Computational Linguistics*, 3214–3252.
...
\newblock *arXiv preprint arXiv:2304.03279*.
...
\newblock *Journal of Consulting Psychology*, 24(4), 349–354.
...
\newblock *Journal of Personality and Social Psychology*, 77(6), 1121–1134.
...
\newblock In *Decision Making and Change in Human Affairs* (pp. 275–324). Springer.

\bibitem[Kadavath et al.(2022)]{kadavath2022}
Kadavath, S., et al. (2022).
\newblock Language Models (Mostly) Know What They Know.
\newblock *arXiv preprint arXiv:2207.05221*.
